...
if (AES_KEY_BASE64) {
    try {
        ...
        if (AES_KEY.length !== 32) {
            console.error('❌ AES_KEY must be exactly 32 bytes (256-bit). Current length:', AES_KEY.length);
            console.error('⚠️  Generate a valid key with: node -e "console.log(require(\'crypto\').randomBytes(32).toString(\'base64\'))"');
            AES_KEY = null;
        } else {
            console.log('✅ AES-256-GCM encryption enabled for file uploads');
        }
    } catch (error) {
        console.error('❌ Invalid AES_KEY_BASE64 format:', error.message);
        AES_KEY = null;
    }
} else {
    console.log('ℹ️  AES_KEY_BASE64 not set - file encryption disabled');
    console.log('⚠️  To enable encryption, add to .env: AES_KEY_BASE64=<base64-encoded-32-byte-key>');
}
    ...
    try {
        ...
        db.run(`
            INSERT INTO activity_logs (
                timestamp, application, window, username, computer_name
            ) VALUES (?, ?, ?, ?, ?)
        `, [
            log.timestamp,
            log.application,
...
app.post('/upload', upload.single('file'), (req, res) => {
    ...
    if (!AES_KEY) {
        return res.status(500).json({ 
            error: 'File encryption not configured', 
            message: 'AES_KEY_BASE64 must be set in environment variables' 
        });
    }
    ...
    try {
        ...
        stmt.run(
            ...
            function(err) {
                ...
                console.log(`✅ File encrypted and uploaded: ${req.file.originalname} (ID: ${this.lastID})`);
                res.json({ 
                    success: true, 
                    id: this.lastID,
...
app.get('/download/:id', (req, res) => {
    ...
    if (!AES_KEY) {
        return res.status(500).json({ 
            error: 'File encryption not configured',
            message: 'Cannot decrypt files without AES key' 
        });
    }
    ...
    db.get('SELECT filename, mimetype, data, iv, auth_tag FROM files WHERE id = ?', [id], (err, row) => {
        ...
        if (!row) {
            return res.status(404).json({ error: 'File not found', id: id });
        }
        ...
        try {
            ...
            console.log(`✅ File decrypted and downloaded: ${row.filename} (ID: ${id})`);
            res.send(decrypted);
            ...
            res.status(500).json({ 
                ...
                details: 'File may be corrupted or encrypted with different key',
...
let connectedUsers = new Map(); // Key: username@computer_name, Value: {username, computer_name, last_seen, status}
...
setInterval(() => {
    ...
    for (const [key, user] of connectedUsers.entries()) {
        if (now - user.last_seen > timeout) {
            connectedUsers.delete(key);
            io.emit('user_disconnected', { username: user.username, computer_name: user.computer_name });
        }
    }
}, 30000);

app.post('/api/heartbeat', (req, res) => {
    const { computer_name, username, status } = req.body;
    
    if (!computer_name || !username) {
        return res.status(400).json({ error: 'Missing required fields' });
    }
    
    const key = `${username}@${computer_name}`;
    const userData = {
        username,
        computer_name,
    ...
    const isNewUser = !connectedUsers.has(key);
    connectedUsers.set(key, userData);
